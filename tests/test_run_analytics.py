...
def _build_test_pipelines(call_log: list[str]) -> tuple[dict[str, object], ...]:
...
def test_parse_selection_supports_numbers_and_names(monkeypatch) -> None:
...
def test_prompt_for_selection_retries_after_invalid_input(monkeypatch) -> None:
...
def test_main_runs_selected_pipelines_once(monkeypatch, capsys) -> None:
...
def test_main_all_runs_every_pipeline(monkeypatch) -> None:
